...
基岩版指令的完整参考。涵盖语法、参数类型、权限等级、版本信息，以及目标选择器、坐标系等基础知识。
...
除指令外，`references/` 目录还包含以下基础知识文件，在编写或理解指令时按需查阅：

| 文件 | 内容 | 何时查阅 |
|---|---|---|
| `target-selector.md` | 目标选择器的类型、参数及用法 | 涉及 `@a`、`@e`、`@s`、`@p`、`@r` 等选择器，或需要筛选实体时 |
| `coordinate-system.md` | 世界坐标系（绝对/相对坐标 `~`）与视角坐标系（局部坐标 `^`） | 涉及坐标填写、相对坐标偏移、视角方向定位时 |
